        ...
        if ($para2 == "free_members") {
            ...
            elseif ($this->member_permission() == TRUE) {
                ...
                if (!empty($ignored_ids)) {
                    $login_user=$this->db->get_where('member',array('member_id' => $member_id))->row();
                    $array_data = array('membership' => 1, 'member_id !=' => $member_id, 'is_blocked' => 'no','is_closed' => 'no');;
                    $array_data = status($member_approval, $array_data);  
                    $this->db->where('gender', $member_gender);
                    $config['total_rows'] = $this->db->where_not_in('member_id', $ignored_ids)->where_not_in('member_id', $ignored_by_ids)->get_where('member', $array_data, $login_user->photo_gallery)->num_rows();
                ...
                // die();
                    ...
                    $array_data = status($member_approval, $array_data);
                    $this->db->where('gender', $member_gender);
                    $config['total_rows'] = $this->db->where_not_in('member_id', $ignored_by_ids)->get_where('member', $array_data, $login_user->photo_gallery)->num_rows();
        ...
        elseif ($para2 == "search") {
            ...
            elseif ($this->member_permission() == TRUE) {
                ...
                if (!empty($ignored_ids)) {
                    ...
                    $array_data = status($member_approval, $array_data);
                    $this->db->where('gender', $member_gender);
                    $all_id = $this->db->select('member_id')->where_not_in('member_id', $ignored_ids)->where_not_in('member_id', $ignored_by_ids)->get_where('member',$array_data, $login_user->photo_gallery)->result();
                    ...
                    $array_data = status($member_approval, $array_data);
                    $this->db->where('gender', $member_gender);
                    $all_id = $this->db->select('member_id')->where_not_in('member_id', $ignored_by_ids)->get_where('member', $array_data, $login_user->photo_gallery)->result();
                }
        ...
        if ($para2 == "free_members") {
            ...
            elseif ($this->member_permission() == TRUE) {
                ...
                if (!empty($ignored_ids)) {
                    ...
                    $this->db->order_by('member_id','desc');
                    $this->db->where('gender', $member_gender);
                    $page_data['get_all_members'] = $this->db->where_not_in('member_id', $ignored_ids)->where_not_in('member_id', $ignored_by_ids)->get_where('member', $array_data, $login_user->photo_gallery, $para1)->result();
                ...
                // die();
                    ...
                    $login_user=$this->db->get_where('member',array('member_id' => $member_id))->row();
                    // print_r($login_user->photo_gallery);
                    // die();
                    $array_data = array('membership' => 1, 'member_id !=' => $member_id, 'is_blocked' => 'no','is_closed' => 'no');
                    $array_data = status($member_approval, $array_data);
                    $this->db->order_by('member_id','desc');
                    $this->db->where('gender', $member_gender);
                    $page_data['get_all_members'] = $this->db->where_not_in('member_id', $ignored_by_ids)->get_where('member', $array_data , $login_user->photo_gallery, $para1)->result();
